...
/*
 * For wchar_t may be of 16 bits on some platforms, use uint32_t instead.
 *
 */
...
int mk_wcswidth(const UTF16 *utf16_str, size_t n)
{
  int w, width = 0;

  // FIXME: only BMP is supported here.
  for (;*utf16_str && n-- > 0; utf16_str++)
    if ((w = mk_wcwidth(*utf16_str)) < 0)
      return -1;
    else
      width += w;
...
int mk_wcswidth(const UTF16 *utf16_str, size_t n, int max_width, int &element_consumed) {
...
int mk_advance_one_code_point(const UTF16 **p, const UTF16 *end) {
  ...
  // TODO_UTF16: FIXME: only support BMP here.
  *p = *p + 1;
  return get_str_width(**p);
}
...
#define USE_CJK_WIDTH


// TODO_UTF16: if we don't use CJK width, we will get wrong text layout.
#ifdef USE_CJK_WIDTH
...
int get_str_width(const UTF16 *utf16_str, size_t n) {
  return mk_wcswidth_cjk(utf16_str, n);
}

int get_str_width(const UTF16 *utf16_str, size_t n, int max_width, int &element_consumed) {
  return mk_wcswidth_cjk(utf16_str, n, max_width, element_consumed);
}
...
int get_str_width(const UTF16 *utf16_str, size_t n) {
  return mk_wcswidth(utf16_str, n);
}

int get_str_width(const UTF16 *utf16_str, size_t n, int max_width, int &element_consumed) {
  return mk_wcswidth(utf16_str, n, max_width, element_consumed);
}
